...
После создания ДО API Контур.Реестро на основании полученных данных проверяет наличие требований (:doc:`requirements<response/requirements>`)  к данному ДО. 
  ...
  * если они определены, то пользователю возвращаются :doc:`response/requirements`.

*************
Актуализация состояния документооборота
*************

После создания документооборота, его состояние может манятся в связи с его обработкой API Реестро, затем Росреестром.
...
Чтобы получить результирующие документы, необходимо выполнить :doc:`method/getfile`.
